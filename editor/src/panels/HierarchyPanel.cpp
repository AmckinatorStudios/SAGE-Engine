...
void HierarchyPanel::DrawNode(EditorHost& host, Scene& scene, entt::entity e) {
    ...
    // У ПАПКИ ВЫКЛЮЧАТЕЛЯ НЕТ, И ЭТО НЕ ОГРАНИЧЕНИЕ, А СОГЛАСОВАННОСТЬ. Папка
    // заведена, чтобы навести порядок в списке, и ничего не меняет в игре —
    // так написано на кнопке, которой её создают. Глаз у папки это обещание
    // ломал: гаснет по цепочке ВСЁ поддерево, то есть один случайный щелчок
    // убирал из кадра десяток объектов. Причину найти было нечем — пропавшие
    // объекты остаются в списке на своих местах, а виноват значок у строки, на
    // которую никто не смотрел. Выключать надо объекты, их и видно.
    if (!scene.IsFolder(e)) {
