...
private:
    ...
    // Ссылка на документ интерфейса: путь, порядок, и что значат его команды.
    // Самого интерфейса здесь нет и быть не должно — его правит редактор
    // документа (панель UI Document). Инспектор сцены отвечает на другой
    // вопрос: КАКОЙ экран здесь показывается и что делают его кнопки.
    void DrawUIDocument(EditorHost& host, GameObject obj);
