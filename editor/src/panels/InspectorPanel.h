...
public:
    ...
    // СОСТАВ МЕНЮ «ДОБАВИТЬ КОМПОНЕНТ»: категория -> имена компонентов, в том
    // же порядке, в каком меню их рисует.
    //
    // Публично — потому что это ДОГОВОР, а не подробность рисования. Меню
    // сворачивается по категориям, и свойство «записи одной категории идут
    // подряд» перестало быть косметикой: запись, вставленная не в свою группу,
    // открыла бы категорию вторым заголовком. Проверить это глазами нельзя —
