...
void ViewportPanel::Draw(EditorHost& host, bool* open) {
    ...
    if (!host.InPlayMode() && slotDrawList[m_activeSlot]) {
        ...
        auto drawMarker = [&](entt::entity e, const char* icon, const glm::vec3& color) {
            ...
            // ПОДЛОЖКА БОЛЬШЕ ЗНАЧКА ПО ДИАГОНАЛИ, а не по стороне.
            //
            // Радиус считался от стороны квадрата значка (0.62), а рисунок
            // занимает весь квадрат целиком — и его углы (лучи лампы, края
            // камеры) торчали из кружка во все стороны. Выглядело это как
            // клякса, у которой не разобрать ни формы, ни границы. Угол
            ...
            // радиуса и на маленьком кружке даёт заметный многоугольник.
            dl->AddCircleFilled(mid, r, IM_COL32(18, 20, 26, 205), 32);
            // Кольцо: на тёмном полу тёмная подложка сливалась с фоном, и
            // значок висел в воздухе. У выбранного кольцо акцентное — та же
            // подсветка, что и в иерархии, но не заливкой: заливка съедала бы
            // сам значок.
            dl->AddCircle(mid, r,
                          selected ? ImGui::GetColorU32(ImGuiCol_NavHighlight)
                                   : IM_COL32(255, 255, 255, 45),
                          32, selected ? 2.0f : 1.0f);
