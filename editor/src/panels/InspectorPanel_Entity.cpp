...
namespace {
// Две строки пояснения одной подсказкой. Разбивать их на две всплывашки нечем —
// подсказка у элемента одна, а разносить смысл по разным полям значит заставить
...
//
// ИНТЕРФЕЙС РИСУЕТСЯ ИЗ ОПИСАНИЙ ЗВЕНЬЕВ, а не отдельным кодом на каждый
// эффект. У звена объявлены параметры (тип, диапазон, подпись, подсказка), и
// панель просто раскладывает их по порядку. Поэтому новое звено — в том числе
// звено, принесённое ИГРОЙ, — появляется здесь само, без правки этой панели.
// Раньше на каждый эффект был свой десяток строк, и «добавить эффект» означало
// «дописать панель».

// Подпись звена или параметра приходит из ДВИЖКА, то есть строкой во время
// работы, а не литералом в этом файле. T() такую строку переводит (ключ — сам
// английский текст), а если перевода нет — отдаёт как есть, и ключ попадает в
// MissingKeys(): непереведённая подпись видна, а не молчит.
static void DrawPostParam(EditorHost& host, sage::render::PostEffect& effect,
                          const sage::render::PostParamDesc& desc) {
    ...
    case PostParamType::Int: {
        ...
        if (ImGui::DragInt(label, &number, 1.0f, (int)desc.Min, (int)desc.Max)) value->V[0] = (float)number;
        host.TrackLastImGuiItem();
        break;
    }
    ...
    default: {
        ...
        if (ImGui::DragFloat(label, &number, step, desc.Min, desc.Max)) value->V[0] = number;
        host.TrackLastImGuiItem();
        break;
    }
    }
...
static void DrawPostChainSection(EditorHost& host, entt::registry& reg, entt::entity entity,
    ...
    for (size_t i = 0; i < component->Chain.Effects.size(); ++i) {
        ...
        if (ImGui::TreeNodeEx("##effect", ImGuiTreeNodeFlags_SpanAvailWidth, "%s",
            ...
            if (kind) {
                ...
                for (const PostParamDesc& param : kind->Params)
                    DrawPostParam(host, effect, param);
            } else {
                // Звено неизвестного вида — из файла, сделанного другой версией
                // или другой игрой. Оно не пропадает молча: его видно, и
                // компилятор о нём уже сказал выше.
        ...
        ImGui::SameLine();
        ImGui::BeginDisabled(i + 1 >= component->Chain.Effects.size());
    ...
    if (removeAt >= 0) {
        host.PushUndoSnapshot();
        component->Chain.Effects.erase(component->Chain.Effects.begin() + removeAt);
    } else if (moveFrom >= 0 && moveTo >= 0) {
        host.PushUndoSnapshot();
        std::swap(component->Chain.Effects[(size_t)moveFrom], component->Chain.Effects[(size_t)moveTo]);
    }
    ...
    if (ImGui::BeginCombo(T("Add effect"), T("Choose an effect"))) {
        for (const PostEffectKind* kind : PostEffectCatalog::Instance().All()) {
            bool already = false;
            for (const PostEffect& e : component->Chain.Effects)
                if (e.Kind == kind->Id) already = true;
            ...
            if (ImGui::Selectable(T(kind->Label))) {
                ...
                // отказ компилятора».
                AddPostEffect(component->Chain, kind->Id);
            }
...
void InspectorPanel::DrawEntityProperties(EditorHost& host) {
    ...
    if (reg.all_of<CameraComponent>(obj.Entity()) && EditorTheme::SectionHeader(T("Camera" "###Camera"), ImGuiTreeNodeFlags_DefaultOpen, &rmCamera)) {
        if (CameraComponent* cam = reg.try_get<CameraComponent>(obj.Entity())) {
            ...
            EditorTheme::Hint(T("Game panel renders from the first Primary camera"));
        }
    }
    ...
    // нет. Раскрывающаяся секция с пустым телом выглядела бы сломанной, поэтому
    // он и не секция: строка состояния плюс кнопка «убрать» рядом.
    if (reg.all_of<NetReplicatedComponent>(obj.Entity())) {
        ImGui::AlignTextToFramePadding();
        ImGui::TextUnformatted(T("Net Replicated"));
        EditorTheme::Hint(Hint2(T("The server sends this object to clients; they follow it"),
    ...
    //
    // Не там, где нажали: кнопка стоит в заголовке, а под ней рисуются поля
    // того же компонента. Сняв его сразу, мы выдернули бы из-под них данные —
    // на том же кадре. Здесь же собраны и «хвосты» снятия: у анимации это
    // возврат статического меша, у звука — глушение источника.
    // МЕШ СНИМАЕТСЯ, КАК ЛЮБОЙ ДРУГОЙ КОМПОНЕНТ. Он был единственным без кнопки
    // «убрать»: добавить его через «Добавить компонент» было можно, а снять —
    // нельзя, и объект, которому меш достался по ошибке (например, брошенной
    // моделью), оставался с ним навсегда. Никакой особости у него нет: это
    // такое же свойство предмета, как свет или звук, и невидимый объект —
    // законное состояние (узел, точка привязки, держатель скрипта).
    if (rmMesh) {
        host.PushUndoSnapshot();
        reg.remove<MeshRendererComponent>(obj.Entity());
    }
    if (rmCamera) {
        host.PushUndoSnapshot();
    ...
    if (rmAnimation) {
        ...
        // Статический меш анимированному объекту не грузится (см. загрузчик
        // сцены): рисует его скелетный проход. Сняли анимацию — рисовать
        // станет некому, и объект пропал бы из кадра до перезагрузки сцены.
        // Догружаем здесь же.
    ...
    if (rmAudio) {
        host.PushUndoSnapshot();
        // Глушим ДО снятия компонента: дескриптор звука живёт в нём, и после
        // remove остановить источник было бы нечем — водопад продолжал бы
        // шуметь из точки, где его уже нет.
        if (AudioSourceComponent* au = reg.try_get<AudioSourceComponent>(obj.Entity())) au->Stop();
