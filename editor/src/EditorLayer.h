...
private:
    ...
    // ОБЩИЕ ПАНЕЛИ ПОМНЯТ СВОЁ МЕСТО В КАЖДОМ ПРОСТРАНСТВЕ.
    //
    // Ассеты и консоль стоят и в сцене, и в вёрстке — но окно у них ОДНО, а
    // док-пространства разные, и докнуть одно окно в два узла нельзя. Пока
    // этого не было, раскладка вёрстки, построившись, забирала ассеты себе:
    // вернувшись в сцену, человек находил панель висящей отдельным окном
