...
public:
    ...
    // Забыть историю пост-обработки (адаптацию глаза, матрицу для смаза):
    // следующий кадр посчитается как первый. Нужна проверкам, которые
    // сравнивают кадры между собой: привыкание глаза меняет кадр законно, и
    // скорость его — от настенных часов, а не от номера кадра.
    ...
    // То же для ВЬЮПОРТА. Запасной путь обложки сцены: у сцены без игровой
    // камеры игрового кадра нет вовсе, и выбор стоит не между «чистой» и
    // «рабочей» картинкой, а между рабочей и никакой.
    bool SaveViewportFrame(const std::string& path);

    // Пиксели ИТОГОВОГО кадра вьюпорта — того самого, что видит человек (после
    // пост-обработки, если она отработала). RGB, строки снизу вверх, как отдаёт
    // RHI.
    //
    // Зачем это вообще нужно: без чтения кадра «картинка во вьюпорте
    // испортилась» проверяется исключительно глазами, а глаз не замечает
    // изменения на несколько процентов за кадр — ровно так и жила
    // накапливающаяся деградация, из-за которой кадр темнел, пока не становился
