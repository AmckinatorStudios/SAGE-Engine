...
void EditorSceneRenderer::RenderViewport(Scene& scene, Camera& camera, const LightingEnvironment& env,
    ...
    // СЕТКА — ШЕЙДЕРОМ, А НЕ ЛИНИЯМИ (sage::render::GridRenderer).
    //
    // Линейная сетка задаёт вопрос, на который нет хорошего ответа: сколько
    // линий рисовать. Сколько ни рисуй, они кончаются — и человек видит ровно
    // то, чего в опорной сетке быть не должно: границу квадрата, вторую границу
    // там, где мелкая клетка сменяется крупной, и пустоту за ними, стоит
    // отлететь или опуститься ниже пола. Здесь вместо линий один полноэкранный
    // треугольник: шейдер пересекает луч с плоскостью и считает линию
    // аналитически, по производной координаты, — толщина выходит постоянной в
    // пикселях, у горизонта сетка не рябит, а гаснет, и «бесконечная» она в
    // буквальном смысле, потому что её протяжённость ничего не стоит.
    //
    // Рисуется ЗДЕСЬ, вместе со сценой (до пост-обработки): сетка — часть
    // кадра, и сглаживать её должно то же экранное сглаживание, что и всё
    // остальное. Гизмо — наоборот, инструмент поверх картинки, и они идут
    // после поста (см. ниже).
    if (showGrid) {
