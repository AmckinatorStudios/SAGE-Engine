...
public:
    ...
    // Отпустить ВСЕ именованные буферы разом — смена проекта.
    //
    // Ключ обложки — путь ассета («assets/hero.fbx»), а такой путь в каждом
    // проекте свой и ведёт к РАЗНЫМ файлам. Пока буферы переживали смену
    // проекта, панель ассетов нового проекта показывала обложки старого: ключ
    // совпал, картинка осталась прежней. Это и есть «призрак прошлого
    // проекта» — с той неприятной чертой, что выглядит он правдоподобно.
    void ForgetProject();
