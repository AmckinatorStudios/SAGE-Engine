...
bool DrawValue(EditorHost& host, const char* id, Value& value, const Var* meta,
    ...
    switch (value.Type()) {
        case Kind::Bool: {
            bool v = value.AsBool();
            if (ImGui::Checkbox("##v", &v)) { value = Value(v); changed = true; }
            break;
        }
        case Kind::Int: {
            int v = value.AsInt();
        ...
        case Kind::Color: {
            ...
            if (ImGui::ColorEdit4("##v", &v.x)) { value = Value(v); changed = true; }
            break;
        }
        case Kind::Entity: {
            sage::vars::EntityRef ref = value.AsEntity();
            if (DrawEntityRef(host, "ref", ref)) { value = Value(ref); changed = true; }
            break;
        }
        case Kind::Asset: {
            // Тот же слот, что у меша и материала: обложка, приём броска с
            // проверкой типа, «показать в Assets».
...
bool DrawTable(EditorHost& host, GameObject obj, Table& table, AssetPreview* preview) {
    ...
    for (size_t i = 0; i < vars.size(); ++i) {
        ...
        } else if (InputText("##name", var.Name)) {
            changed = true;
        }
        ImGui::SameLine();

        // Вид переменной меняется только у заведённых руками — по той же
        // причине: вид объявленной задан скриптом.
        if (!var.Declared) {
            ...
            if (ImGui::Combo("##kind", &current, names.data(), (int)names.size())) {
                // Смена вида СОХРАНЯЕТ введённое, насколько возможно: обнулять
                // — терять уже набранное из-за одного щелчка.
        ...
        ImGui::SameLine();
        // Объявленную скриптом удалить нельзя — она вернётся при следующем
        // чтении объявления, и кнопка обманывала бы.
        if (!var.Declared && EditorIcons::Button("trash", "", T("Remove the variable"))) {
            removeAt = (int)i;
        }
        ImGui::PopID();
    }
...
bool DrawBindings(EditorHost& host, const char* id, sage::events::Bindings& bindings,
    ...
    for (size_t i = 0; i < bindings.size(); ++i) {
        ...
        if (open) {
            if (ImGui::Checkbox(T("Enabled##binding"), &b.Enabled)) changed = true;
            ...
            // Кому лично.
            ImGui::TextUnformatted(T("Call on object"));
            ImGui::SameLine();
            ...
            ImGui::SetNextItemWidth(140.0f);
            if (ImGui::Combo(T("Argument"), &kind, kindNames.data(), (int)kindNames.size())) {
