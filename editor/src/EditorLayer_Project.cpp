...
bool EditorLayer::BuildGame(const fs::path& outputDir, std::string& err) {
    ...
    // ИГРА БЕЗ СЦЕНЫ — НЕ ИГРА, и узнать об этом надо ЗДЕСЬ.
    //
    // Сборка честно упаковывала проект, у которого в scenes/ пусто, и выдавала
    // папку с exe: снаружи — удача, «игра собрана». Отказ случался у ИГРОКА, на
    // его машине, в виде окна «В проекте нет ни одной сцены» — то есть на шаг
    // позже, чем нужно, и не у того человека.
    //
    // Случай не редкий и не выдуманный: человек создаёт проект, что-то ставит в
    // сцену и жмёт «Собрать игру», ни разу не сохранив её (File > Save Scene).
    // Сцена живёт в памяти редактора, на диске её нет, и пакет получается
    // пустым. Поэтому отказываем до сборки и говорим, что именно сделать.
    if (!m_project.HasScenes()) {
        err = T("The project has no scenes, so there is nothing to build. Save the scene first (File > Save Scene).");
        LOG_ERROR("Editor") << "Сборка игры отклонена: в проекте нет ни одной сцены ("
                            << m_project.ScenesDir().string() << ")";
        return false;
    }
