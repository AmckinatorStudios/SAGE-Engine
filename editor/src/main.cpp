...
sage::Application* sage::CreateApplication(int /*argc*/, char** /*argv*/) {
    ...
    // РАЗВЁРНУТО НА ВЕСЬ ЭКРАН — и стартовое окно проектов, и сам редактор (это
    // одно окно). Полтора десятка панелей в окне 1600x900 показываются
    // огрызками: дерево сцены в три строки, инспектор, у которого половина
    // полей ниже края. Первое действие каждого запуска — растянуть окно; делать
    // это за человека и есть работа программы. Размер 1600x900 остаётся
    // размером окна, когда его свернут из развёрнутого.
    ...
    if (const char* w = std::getenv("SAGE_EDITOR_WINDOW")) {
        ...
        if (std::sscanf(w, "%dx%d", &ww, &hh) == 2 && ww >= 640 && hh >= 480) {
            config.Width = ww;
            config.Height = hh;
            // Заказанный размер и разворот несовместимы: развёрнутое окно
            // получит размер монитора, и проверка снимет кадр не того размера,
            // который просила.
