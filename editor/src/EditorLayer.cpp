...
void EditorLayer::OnUpdate(float dt) {
    // Правки проекта снаружи (SDK, скрипт, второй редактор) — до всего
    // остального в кадре: остальная логика кадра должна работать уже с
    // актуальным состоянием, а не с тем, что осталось от прошлого.
    PollProjectChanges(dt);
