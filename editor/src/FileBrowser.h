...
public:
    ...
    struct Config {
        ...
        // ГРАНИЦА: выше этой папки диалог не пускает. Пусто — весь диск.
        //
        // ЗАЧЕМ. У диалога две разные работы, и путать их нельзя.
        //
        //   • ВНЕСТИ В ПРОЕКТ (импорт, открыть проект, папка сборки) — работа
        //     СНАРУЖИ: файл лежит в «Загрузках», на флешке, у соседа в папке.
        //     Границы здесь быть не может, иначе вносить нечего.
        //   • ВЫБРАТЬ АССЕТ ПРОЕКТА (материал, скрипт, текстура, модель, звук)
        //     — работа ВНУТРИ: выбирают из того, что в проекте уже есть.
        //
        // Пока границы не было, второе делалось первым: в слот материала можно
        // было положить картинку из «Загрузок». Ссылка на файл вне проекта
        // работает ровно до первой сборки игры и до переноса проекта на другую
        // машину — а потом молча превращается в пустой слот, и объяснить это
        // человеку нечем: у него на диске файл на месте.
        //
        // Это та же граница, что у панели Assets, и ровно то же правило, что в
        // CLAUDE.md: ассет выбирается ВНУТРИ проекта, наружу смотрят только
        // папка проекта, создание/открытие проекта и папка сборки.
    ...
    // Где диалог сейчас стоит. Нужен самопроверке: «диалог не выпускает за
    // границу» — утверждение о ТЕКУЩЕЙ папке, и проверить его иначе как
    // спросив её нечем.
    const std::filesystem::path& CurrentDir() const { return m_dir; }
