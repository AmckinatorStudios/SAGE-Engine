...
public:
    ...
    //
    // false — система отказалась открывать (нет файла, нет назначенной
    // программы). Отказ обязан быть виден: молчащий двойной клик неотличим от
    // сломанного редактора.
    virtual bool OpenFileInSystemEditor(const std::filesystem::path& path) = 0;
