...
//
// Имя с суффиксом Id намеренно: EditorPanel — это базовый класс панели на
// SAGE UI (ui/EditorPanel.h, §29 ТЗ), а здесь перечислены её ИДЕНТИФИКАТОРЫ.
// Одно имя на два разных понятия в соседних заголовках — ловушка для всякого,
// кто включит оба.
enum class EditorPanelId {
    ...
    Game,
    Viewport,
    UIDocument,    // редактор документа интерфейса (.uidoc)
...
public:
    ...
    virtual bool& PanelVisible(EditorPanelId panel) = 0;
