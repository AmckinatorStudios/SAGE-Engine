...
public:
    ...
    // ВКЛЮЧЕНО ЛИ СМЕШИВАНИЕ СЕЙЧАС.
    //
    // Запрос, а не «и так знаем»: смешивание — это состояние, которое каждый
    // проход поднимает под себя и обязан снять за собой, а забытая строка не
    // ломает ни сборку, ни ближайший кадр. Она проявляется через несколько
    // проходов и через несколько кадров: цепочка пост-обработки пишет в буферы,
    // живущие между кадрами, и со включённым смешиванием начинает подмешивать
    // кадр к предыдущему — картинка темнеет всё сильнее, а виноватый проход
    // находится где угодно, только не там, где видно поломку.
    //
    // С запросом это перестаёт быть догадкой: правило «проход возвращает
    // состояние» проверяется тестом после каждого прохода (см.
    // tests/render/Checks_Stability.cpp).
    virtual bool BlendEnabled() const = 0;
