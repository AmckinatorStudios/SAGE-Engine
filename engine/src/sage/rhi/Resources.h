...
public:
    ...
    // То же, но для ДИАПАЗОНА индексов — подмеша внутри общего буфера.
    //
    // Нужна многоматериальным моделям: один файл модели приезжает одним
    // вершинным буфером, но части его геометрии красятся РАЗНЫМИ материалами, и
    // каждая такая часть — свой отрезок индексного буфера. Без диапазона
    // пришлось бы либо резать модель на отдельные Geometry (и потерять
    // инстансинг вместе с общим буфером), либо красить её целиком одним
    // материалом — то есть ровно то, из-за чего модель с четырнадцатью
    // материалами выглядела одноцветной.
    virtual void DrawIndexedInstancedRange(size_t firstIndex, size_t indexCount,
                                           size_t instanceCount) const = 0;
...
struct Texture2DDesc {
    ...
    // Пиксели — ЦВЕТ в sRGB (альбедо, свечение): при выборке видеокарта
    // переводит их в линейные. Освещение считается в линейном пространстве, а
    // кадр на выходе кодируется в sRGB; без перевода на входе цвет картинки
    // кодировался бы дважды — коричневая кора выходила серо-розовой, листва
    // выцветшей, и вся сцена выглядела блёклой. Данные (нормали, шероховатость,
    // маски) — НЕ цвет, для них флаг не ставится. Только 3 и 4 канала.
    bool Srgb = false;
};
