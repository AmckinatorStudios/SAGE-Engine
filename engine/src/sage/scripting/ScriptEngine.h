...
public:
    ...
    // ГОРЯЧАЯ ПЕРЕЗАГРУЗКА: перечитать те скрипты, ФАЙЛЫ которых изменились.
    // Возвращает, сколько перечитано.
    //
    // ЗАЧЕМ. Скрипт правят во внешнем редакторе (своего в SAGE нет намеренно), и
    // до сих пор правка не значила НИЧЕГО, пока не остановишь и не запустишь
    // игру заново. То есть цикл «поправил число — посмотрел» стоил перезапуска
    // уровня: дойти до того же места, набрать тот же счёт, снова встать перед
    // тем же врагом. Шейдеры так умеют давно (ResourceManager::
    // ReloadChangedShaders), а скрипты — нет, хотя правят их куда чаще.
    //
    // ЧТО ПРОИСХОДИТ С СОСТОЯНИЕМ. Скрипт перепривязывается, то есть его
    // окружение создаётся заново и OnStart выполняется снова. Локальные
    // переменные скрипта обнуляются — иначе новый код видел бы половину старых
    // данных, и поведение зависело бы от того, что именно успели поправить.
    // Сцена, сущности и мировые переменные при этом не трогаются.
    //
    // ОШИБКА В ПРАВКЕ НЕ ЛОМАЕТ ИГРУ. Скрипт с опечаткой не компилируется —
    // прежний экземпляр остаётся работать, а причина уходит в лог. Тот же
    // порядок, что у шейдеров: недописанная строка не имеет права выключить то,
    // что уже работало.
    ...
    // прогресса за время с последнего автосохранения: человек что-то строил
    // двадцать минут, вышел через меню — и обнаружил постройку такой, какой она
    // была полминуты назад. Причём молча: ни ошибки, ни предупреждения, всё
    // «работает».
    //
...
private:
    struct ScriptInstance {
        ...
        // Время правки файла на момент загрузки. По нему ReloadChangedScripts
        // понимает, что снаружи скрипт переписали. 0 — файла на диске нет
        // (скрипт приехал из .sagepak собранной игры): там перезагружать
        // нечего и не от чего.
        long long Stamp = 0;
