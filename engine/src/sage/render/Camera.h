...
public:
    ...
    // Поставить взгляд в ЗАДАННЫЕ углы. Отдельно от ProcessMouse, потому что
    // это не «поверни на столько-то», а «встань ровно так»: гизмо осей и виды
    // «спереди/сверху/сбоку» ставят камеру в точный угол, а не докручивают её
    // мышью. Без этого им пришлось бы трогать Yaw/Pitch напрямую и звать
    // пересчёт базиса — то есть знать про внутреннее устройство камеры.
    void SetAngles(float yawDeg, float pitchDeg) {
        Yaw = yawDeg;
        // 90° ровно — вырожденный случай: Front становится параллелен WorldUp,
        // и векторное произведение для Right вырождается в ноль.
        Pitch = pitchDeg > 89.9f ? 89.9f : (pitchDeg < -89.9f ? -89.9f : pitchDeg);
        UpdateVectors();
    }
