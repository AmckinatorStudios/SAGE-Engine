...
public:
    ...
    //
    // ЭТО КОНЕЧНЫЙ КВАДРАТ, и для опорной сетки вида он не годится: у него
    // видно край, а при смене шага — и границу между частой и редкой клеткой.
    // Такую сетку рисует sage::render::GridRenderer — одним полноэкранным
    // треугольником, аналитически и без края (её и ставит редактор). Здесь
    // осталось то, ради чего примитив и заведён: небольшой квадрат-ориентир в
    // отладочной графике игры.
