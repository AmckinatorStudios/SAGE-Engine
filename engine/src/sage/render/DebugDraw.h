...
public:
    ...
    // БЕСКОНЕЧНАЯ СЕТКА: следует за камерой и меняет шаг вместе с высотой.
    //
    // ЗАЧЕМ. Сетка была квадратом 24 на 24 вокруг начала координат. Стоит
    // отлететь на полсотни единиц — и опоры под ногами нет вовсе: не видно ни
    // горизонта, ни направления, ни масштаба, а собранный вдали кусок уровня
    // висит в пустоте. Это не косметика: без сетки нечем понять, ровно ли стоит
    // объект и насколько он велик.
    //
    // Бесконечной её делают две вещи. Первая: клетка ПРИВЯЗАНА К МИРУ, а центр
    // переезжает за камерой ровно по клеткам — сетка не «едет» под ногами, она
    // стоит, а мы по ней летим. Вторая: шаг растёт вместе с высотой десятками
    // (1, 10, 100), иначе с высоты в километр это сплошная заливка из линий, а
    // у земли — три полоски на весь экран.
    //
    // Мелкая клетка рисуется только вблизи, крупная — до края: так плотность
    // спадает к горизонту, и край сетки не читается обрывом.
    void InfiniteGrid(const glm::vec3& cameraPos, glm::vec3 color);
    ...
    // Капсула в произвольном повороте: цилиндр высотой 2*halfHeight вдоль
    // локальной оси Y плюс полусферы на торцах.
    //
    // Отдельный примитив, а не ящик по габаритам: капсула — самая частая форма
    // персонажа, и рисовать её ящиком значит показывать углы, которых у неё
    // нет. Именно по этим углам и судят, почему герой «цепляется за косяк», —
    // а он за него не цепляется, цепляется гизмо.
    void WireCapsule(const glm::mat4& transform, float radius, float halfHeight, glm::vec3 color,
                     int segments = 16);
