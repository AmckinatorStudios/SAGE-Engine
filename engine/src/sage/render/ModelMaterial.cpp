...
// которой её пишет 3D-редактор: Ns = (30·(1 − roughness))², то есть 0..900. Прежде
...
// Имя, годное для файла: пробелы и разделители из имени материала в 3D-редакторе
...
void ExtractObjMaterial(const fs::path& dir, const tinyobj::material_t& m, bool kdMissing,
    ...
    // map_d — КАРТА прозрачности. 3D-редактор пишет её для листвы, травы и
...
ExtractedMaterialSet ExtractMaterials(const std::string& ref, const std::string& textureDir) {
    ...
    // из 3D-редактора, у которого материал собран узлами (шум, градиент, смешение
    ...
    // что произошло, и куда смотреть: перевыложить материалы из 3D-редактора
    ...
    if (!out.Materials.empty()) {
        ...
        if (!anyMap && !anyColour) {
            out.Warnings.push_back(
                ...
                "Так выходит, когда материал в 3D-редакторе собран узлами: в glTF попадает "
    ...
    // baseColorFactor, 3D-редактор пишет в .mtl и FBX тоже линейные значения своего
