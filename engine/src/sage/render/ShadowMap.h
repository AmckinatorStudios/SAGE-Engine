...
class ShadowMap {
public:
    // Каскадов не больше трёх, и это ограничение не про качество, а про
    // ТЕКСТУРНЫЕ ЮНИТЫ. Каждый каскад — отдельная карта, то есть отдельный
    // сэмплер; OpenGL 3.3 гарантирует шестнадцать юнитов фрагментного шейдера
    // (столько и даёт, например, OpenGL от Apple), и после шести карт
    // материала, трёх текстур GI-объёма, лайтмапы, куба окружения, плоского
    // отражения и атласа локальных теней на солнце остаётся ровно три.
    //
    // Раньше здесь стояло четыре — и четвёртый каскад отбирал юнит у карты
    // самосвечения: uEmissiveMap и uGIVolR оказывались на ОДНОМ юните, а
    // сэмплировать один юнит как sampler2D и sampler3D спецификация объявляет
    // неопределённым поведением. Сцена с запечённым GI-объёмом и светящимся
    // материалом попадала в эту неопределённость молча.
    //
    // Потеря невелика и была описана прямо здесь ещё до этой правки: каждый
    // каскад — это ещё один полный проход отбрасывающей тень геометрии, а
    // выигрыш в детализации падает вдвое с каждым шагом, и четвёртый уходит на
    // дальность, которую в кадре обычно занимает небо.
    //
...
//
// Полная раскладка шестнадцати юнитов — единственное место, где она записана
// целиком, и потому единственное, где видно, что свободных больше нет:
//   0  albedo            6..8  GI-объём (R,G,B)   12  самосвечение
//   1  каскад 0          9     лайтмапа           13  куб окружения
//   2  нормали           10    каскад 1           14  плоское отражение
//   3  металличность     11    каскад 2           15  атлас локальных теней
//   4  шероховатость
//   5  ambient occlusion
constexpr int kShadowUnit0 = 1;
constexpr int kShadowUnitRest = 10; // каскад 1 -> 10, 2 -> 11
constexpr int kEmissiveUnit = 12;
...
struct ShadowBinding {
    ...
    // Тени прожекторов и точечных источников (см. render/ShadowAtlas.h). Живут
    // здесь, а не отдельным параметром отрисовки, ровно по тому же доводу, по
    // которому здесь живут каскады: иначе к каждой функции рендера пришлось бы
    // дописать ещё один аргумент, и первый же забытый вызов дал бы сцену, где
    // солнце тень отбрасывает, а лампа — нет, без единого сообщения об ошибке.
    sage::render::LocalShadowBinding Local;
    ...
    void Bind() const {
        sage::rhi::GraphicsDevice& device = sage::rhi::GraphicsDevice::Get();
        // Атлас локальных теней привязывается НЕЗАВИСИМО от солнечных карт:
