...
// Геометрия glTF без GPU-ресурсов живёт НЕ здесь, а в импортёре
// (assets/import/GltfImporter.cpp). Раньше её собирал отдельный обход в этом
// файле — и собирал в ОДИН безымянный буфер, без границ между частями и без
// материалов: ровно из-за этого модель с четырнадцатью материалами рисовалась
// одним. Импортёр читает те же узлы, но сохраняет их структуру, а склейкой в
// один меш занимается ImportedScene::Flatten(), который границы не теряет.
