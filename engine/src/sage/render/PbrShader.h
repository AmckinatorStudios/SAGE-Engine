...
// Френель С УЧЁТОМ ШЕРОХОВАТОСТИ — для РАССЕЯННОГО света, а не для источника.
//
// Обычный Френель на скользящем угле стремится к единице: «вся энергия ушла в
// отражение». Для источника света это верно, для ambient — нет: у рассеянного
// света нет одного направления, и шероховатая поверхность на скользящем угле
// продолжает рассеивать. Пока доля диффузного считалась обычным Френелем,
// шероховатый бок, повёрнутый к камере ребром, получал НОЛЬ рассеянного света
// и при отсутствии карты окружения становился ЧЁРНЫМ — посреди полностью
// освещённой сцены, без всякой тени. На модели это выглядит как чёрные полосы
// и крапины по краям и на гранях, повёрнутых от камеры.
//
// Формула стандартная (Karis): потолок отражённой доли опущен до
// (1 - шероховатость), то есть матовая поверхность не «зеркалит» даже вскользь.
...
// ТУМАН. Высотный — экспоненциальный высотный туман: плотность
...
// дымка светится его цветом — узнаваемый «воздух» рассвета и заката.
...
//
// ИНТЕНСИВНОСТЬ 1 — ЭТО «БЕЛАЯ ПОВЕРХНОСТЬ ПОД СОЛНЦЕМ БЕЛАЯ» (общепринятая
// калибровка): освещённость источника равна π·интенсивность, и π BRDF Ламберта
...
vec3 ShadePBRplanar(vec3 N, vec3 fragPos, vec3 albedo, float metallic, float rough, float ao,
    ...
    vec3 F0 = mix(vec3(0.04), albedo, metallic);
    vec3 kD = (vec3(1.0) - FresnelSchlickRoughness(max(dot(N, V), 0.0), F0, rough)) *
              (1.0 - metallic);
...
void main() {
    ...
    if (uHasNormal) {
        ...
        vec3 mapped = TBN * n;
        // ВЫРОЖДЕННЫЙ БАЗИС НЕ ИМЕЕТ ПРАВА ГАСИТЬ ПИКСЕЛЬ. normalize нулевого
        // вектора даёт NaN, а NaN в цвете — чёрная точка, которая переживает
        // всё: и освещение, и пост-обработку. Базис вырождается там, где
        // развёртка сходится в точку или касательные соседних вершин смотрят
        // в разные стороны (шов), — и на модели это выглядит как россыпь
        // чёрных крапин, которую не объяснить ни светом, ни материалом.
        if (dot(mapped, mapped) > 1e-8) N = normalize(mapped);
    }
    // ЗАДНЯЯ ГРАНЬ СМОТРИТ НАЗАД. У двустороннего материала (cull=none, а у
    // моделей со сканов и выгрузок это ПОЧТИ ВСЕГДА так) в кадр попадает
    // изнанка поверхности, а нормаль у неё осталась от лицевой стороны — то
    // есть направлена ОТ зрителя. Свет на такую грань не попадает по
    // построению: скалярное произведение с направлением на источник
    // отрицательное, и поверхность выходит ЧЁРНОЙ при полном освещении вокруг.
    //
