...
// КАРТЕ (обычный glTF из 3D-редактора или из магазина моделей), получала множитель
...
//
// Отделён от загрузки намеренно. Во-первых, так его зовёт и кэш, и конвертер, и
// тест: проверять правильность скиннинга и анимации «по картинке» — то же
// самое, что не проверять. Во-вторых, формат выбирается здесь ОДИН раз, а не в
// каждом вызывающем: путь разбора — свойство файла, а не того, кто его открыл.
//
// Бросает std::runtime_error с внятным текстом (файл, этап, причина).
