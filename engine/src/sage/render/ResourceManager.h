...
public:
    ...
    //
    // bleed — отдать прозрачным пикселям цвет соседних непрозрачных (см.
    // render/AlphaBleed.h). Только для карт с вырезом по альфе: у листвы под
    ...
    //
    // Настройки выборки и bleed — те же, что у GetTexture, и дают ту же запись
    // кэша: картинку, заказанную фоном, потом спросят обычным путём. Пока
    // пиксели в пути, у выреза по альфе (bleed) заглушка ПРОЗРАЧНАЯ: серый
    ...
    // включает: там модель ставят в сцену посреди работы, и три карты 2048²
    // (дерево из набора) декодировались прямо в кадре — редактор замирал на
    // полсекунды на каждый бросок модели и на каждую обложку.
    void SetStreamMaterialTextures(bool on) { m_streamMaterialTextures = on; }
    bool StreamMaterialTextures() const { return m_streamMaterialTextures; }
    // Сколько картинок ещё в пути (очередь + декод + ждут заливки).
    int PendingAsyncTextures() const;
    ...
    // снаружи — в 3D-редакторе, в другом редакторе, скриптом, — и тогда движок
    ...
    // переэкспортировал персонажа из 3D-редактора. Редактор при этом показывал
    ...
    //
    // budget — сколько файлов опросить за этот вызов (0 — все). Опрос идёт по
    // кругу: следующий вызов продолжает с того места, где остановился прошлый.
    // ЗАЧЕМ. Редактор зовёт это КАЖДЫЙ кадр, а загруженных ресурсов после
    // просмотра набора ассетов — сотни (модели, материалы, текстуры, обложки).
    // Опрашивать диск по каждому из них в каждом кадре значило платить
    // тысячами системных вызовов за кадр: FPS проседал вдвое (180 -> 90) просто
    // от того, что ассеты когда-то открывали. С бюджетом стоимость кадра
    // постоянна, а правка файла всё равно замечается за доли секунды.
    int ReloadChangedAssets(size_t budget = 0);
...
private:
    ...
    bool m_streamMaterialTextures = false;
    // Где остановился опрос файлов (см. ReloadChangedAssets с бюджетом).
    size_t m_reloadCursor = 0;
