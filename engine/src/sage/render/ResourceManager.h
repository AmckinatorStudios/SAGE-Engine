...
public:
    ...
    // Перечитывает ВСЁ, что загружено с диска и с тех пор изменилось: модели,
    // скелетные модели, материалы и текстуры.
    //
    ...
    //
    // ПОЧЕМУ ВСЕ ЧЕТЫРЕ ВИДА, А НЕ ДВА. Модель и материал перечитывались, а
    // текстура и скелетная модель — нет, и это ровно те два случая, в которых
    // правку делают чаще всего: перерисовал карту в графическом редакторе,
    // переэкспортировал персонажа из Blender. Редактор при этом показывал
    // прежнюю картинку без единого признака того, что он смотрит в старое.
    //
    // Возвращает, сколько ресурсов перечитано. Зовётся редактором раз в кадр
    // рядом с ReloadChangedShaders.
    int ReloadChangedAssets();

    // Номер поколения ресурсов: растёт на каждую перезагрузку (см.
    // ReloadChangedAssets). По нему те, кто держит НЕ shared_ptr, а разобранные
    // данные из ресурса — аниматор с указателями на скелет и клипы, кэш
    // материалов превью, — понимают, что их разбор устарел. Сверять пути и
    // времена правки у себя каждому такому месту значило бы повторить эту
    // проверку столько раз, сколько в редакторе кэшей.
    uint64_t AssetsGeneration() const { return m_assetsGeneration; }
    ...
    // ЗАБЫТЬ ВСЁ, ЧТО ПРИНАДЛЕЖАЛО ПРОШЛОМУ ПРОЕКТУ.
    //
    // Кэш держит ресурсы ПО ССЫЛКЕ ПРОЕКТА («assets/hero.fbx»), а такая ссылка
    // в каждом проекте своя и указывает на РАЗНЫЕ файлы. Пока кэш переживал
    // смену проекта, новый проект получал чужую модель, чужую текстуру и чужой
    // материал — те, что первыми легли под это имя. Снаружи это «призрак
    // прошлого проекта»: в новом проекте на секунду видно работу из старого.
    //
    // ЧЕМ ОТЛИЧАЕТСЯ ОТ Clear(). Clear() — выключение: он снимает у материалов
    // ссылку на шейдерную программу, чтобы та умерла при ЖИВОМ GL-контексте, и
    // останавливает фоновый поток. Звать его на смене проекта нельзя: материалы
    // с оборванной программой ещё держит уходящая сцена. Здесь же просто
    // выбрасываются записи кэша — то, что кто-то ещё держит, доживёт своё и
    // умрёт само.
...
private:
    ...
    void DowngradeTexture(const std::string& key); // понизить разрешение вдвое
    ...
    std::unordered_map<std::string, long long> m_skinnedStamps;
    uint64_t m_assetsGeneration = 0;
    ...
    struct TextureRecord {
        ...
        uint64_t Tick = 0;
        bool Pending = false;
        // ПУТЬ К ФАЙЛУ. Ключ кэша им больше не является: один файл живёт в
        // кэше СТОЛЬКО раз, сколько у него разных настроек выборки (см.
        // CacheKey), и перечитать, понизить или выгрузить запись можно только
        // зная, откуда она взялась.
        std::string Source;
        // С какими настройками картинка на видеокарте. Часть ключа, а не
        // состояние записи: две фильтрации одного файла — это две картинки.
        TextureFilter Filter = TextureFilter::Trilinear;
        bool Mipmaps = true;
