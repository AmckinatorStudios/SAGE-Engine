...
MeshData BuildCapsule(int rings, int sectors) {
    ...
    if (sectors < 3) sectors = 3;
    const float pi = glm::pi<float>();
    // РАДИУС ЧЕТВЕРТЬ, А НЕ ПОЛОВИНА — иначе капсулы не существует.
    //
    // Здесь стояло radius = 0.5 при общей высоте 1, и цилиндрическая часть
    // выходила ровно нулевой: капсула получалась СФЕРОЙ, один в один. В
    // комментарии это называлось «правильным предельным случаем», но форма,
    // которая всегда совпадает с другой формой, не нужна ни в списке, ни в
    // движке — а заводили её как рабочую форму персонажа.
    //
    // Пропорции — как у капсулы в любом редакторе: ширина вдвое меньше высоты
    // (Unity: диаметр 1 при высоте 2). Общая высота остаётся единицей, как у
    // куба и сферы: примитив обязан вписываться в единичный куб, иначе
    // «масштаб 1» означал бы у каждой формы своё.
    const float radius = 0.25f;
    // Половина цилиндрической части: то, что осталось от единичной высоты
    // после двух полусфер.
