...
void EnvironmentMap::CaptureSky(SkyRenderer& sky, const LightingEnvironment& env) {
    ...
    for (int face = 0; face < 6; ++face) {
        m_cube->BindFace(face, 0);
        dev.Clear(true, true);
        sky.Draw(FaceView(face, glm::vec3(0.0f)), proj, env.Skybox.TopColor,
                 env.Skybox.HorizonColor);
    }
    m_cube->GenerateMips();
}
...
void ReflectionSystem::UpdateSky(SkyRenderer& sky, const LightingEnvironment& env) {
    if (!m_enabled) return;
    // Пересъёмка только на изменение цвета неба. Сравнение по значению, а не по
    // флагу «поменялось»: цвета правит и редактор, и скрипт, и загрузка сцены.
    if (m_captured && m_skyTop == env.Skybox.TopColor && m_skyHorizon == env.Skybox.HorizonColor)
        return;
    EnvironmentMap& e = Ensure();
    if (!e.Valid()) return;
    e.CaptureSky(sky, env);
