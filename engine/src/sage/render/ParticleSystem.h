...
public:
    ...
    // ВСЁ ЖИВОЕ — ПРОЧЬ. Нужно при смене сцены и проекта: частица живёт
    // секундами и системе, которая её родила, ничем не обязана. Пока этого не
    // было, залп, пущенный в одном проекте, догорал уже в другом — человек
    // открывал новый проект и видел в нём чужие искры. То же и со стримами:
    // непрерывный эмиттер продолжал сыпать частицы от объекта, которого в новой
    // сцене нет вовсе.
    void Clear() {
        m_particles.clear();
        m_streams.clear();
    ...
    void Draw(Shader& shader, glm::vec3 camRight, glm::vec3 camUp,
        ...
        // И СМЕШИВАНИЕ ТОЖЕ. Запись глубины возвращалась, а смешивание
        // оставалось включённым — и доживало до пост-обработки, чьи буферы
        // затенения и свечения живут между кадрами: полноэкранный проход
        // подмешивался к прошлому кадру вместо того, чтобы его заменить, и
        // картинка темнела с каждым кадром. Проявлялось это не сразу, а с
        // первого же кадра, где нашлась хоть одна частица, — отсюда и
        // «работало, а потом само испортилось».
