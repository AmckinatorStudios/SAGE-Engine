...
// разреженный аксессор, которым 3D-редактор пишет каждый ключ формы) означало
...
static ModelData ParseGltf(const std::string& path) {
    ...
    // от корня сцены, магазины моделей и часть выгрузок 3D-редакторов пишут от корня
...
ModelData ParseSkinnedModelFile(const std::string& path) {
    ...
    // персонаж из FBX (а это всё, что отдают 3D-редакторы, сервисы риггинга и
