...
Application::~Application() {
    ...
    // КЭШ РЕСУРСОВ ЗАКРЫВАЕТСЯ ЗДЕСЬ — пока контекст ещё жив.
    //
    // ResourceManager — функция-статик: сам по себе он разрушится обработчиком
    // выхода процесса, то есть УЖЕ ПОСЛЕ того, как окно и устройство уничтожены
    // (см. m_window.reset() ниже). Освобождение буфера видеокарты в этот момент —
    // вызов по указателю на функцию, которой больше нет: процесс падал в
    // обработчике выхода, когда в логе уже всё написано, отчёт напечатан и
    // причина не видна. Снаружи это «игра отработала и упала» с кодом 139.
    //
    // Обходной путь — звать Clear() у себя в OnDetach — знали все четыре
    // потребителя движка (редактор, плеер, sandbox, testgame), и НИ ОДИН из них
    // не был обязан: знание жило в памяти авторов, а не в движке. Первая же игра,
    // написанная по документации, на этом и упала — см. tests/exit_probe.cpp,
    // который играет ровно её роль. Временем жизни контекста владеет Application,
    // значит он и обязан закрыть кэш, пока контекст жив.
    //
