...
struct EngineConfig {
    ...
    // Число каскадов теней (1..4). Один — правильный ответ для помещения:
    // вся геометрия рядом, и три прохода дали бы ту же картинку втрое дороже.
    // Для улицы нужны каскады: с одной картой размер текселя приходится
    // растягивать на всю даль, и тень под ногами превращается в лесенку.
    // Каждый каскад — это ещё один полный проход отбрасывающей тень геометрии.
