...
// встроенный движок и пустой Null) прячется за фабрикой PhysicsWorld::Create.
// ---------------------------------------------------------------------------
namespace sage::physics {

// Доступные бэкенды.
//
//   Builtin — СОБСТВЕННЫЙ движок SAGE (engine/src/physics/builtin). Без
//             внешних зависимостей, доступен всегда, и делает полную работу
//             твёрдого тела: вращение с честным тензором инерции, повёрнутые
//             коробки и капсулы, манифольды контактов, последовательные
//             импульсы с трением и тёплым стартом, все шесть типов соединений,
//             широкая фаза заметанием, сон тел и спекулятивные контакты.
//   Jolt    — внешний движок (jrouwe/JoltPhysics), подключается через CMake
//             (SAGE_PHYSICS_JOLT). Берёт на себя то, чего у встроенного нет по
//             замыслу: произвольные выпуклые оболочки, треугольные сетки,
//             мягкие тела.
//   Null    — заглушка (физика отключена).
enum class Backend {
    Null,
    Builtin,
    Jolt,
    // Прежнее имя встроенного бэкенда. Оставлено псевдонимом, чтобы игры и
    // конфигурации, написанные до появления полноценного движка, продолжали
    // собираться: ломать чужой работающий код ради переименования — цена без
    // выгоды.
    Simple = Builtin,
};
...
// Форма коллайдера. Капсула — настоящая капсула в обоих бэкендах: и встроенный,
// и Jolt сталкивают её как отрезок с радиусом, а не как коробку.
...
// них StaticCompoundShape; встроенный сталкивает каждую часть отдельно и
// считает по ним общий тензор инерции.
struct ChildShape {
