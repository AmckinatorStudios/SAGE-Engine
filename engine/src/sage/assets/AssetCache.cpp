...
//           и вторая развёртка.
//   3 -> 4: жёсткие детали приводятся к кости иначе (через обратную матрицу
//           палитры в позе привязки, а не через Skeleton::Root). Версия здесь
//           обязательна, хотя ни одно ПОЛЕ не изменилось: в кэше лежат уже
//           ПЕРЕСЧИТАННЫЕ вершины, и старый файл поставил бы детали по старой
//           формуле — модель собиралась бы зеркально, а понять почему было бы
//           нельзя: исходник тот же, код новый, картинка чужая.
//   4 -> 5: скиновый разбор FBX стал брать ВСЮ геометрию, а не только ту, у
//           которой есть веса (жёсткие детали приезжают вырожденным скином), и
//           читать материалы. Поля те же, содержимое другое: в старом кэше
...
// 7: скиновый разбор FBX красит части по материалу ГРАНИ (одна геометрия —
// несколько подмешей), режет многоугольники по плоскости, а не веером,
