...
// бесполезный ответ; «файл сжат (по умолчанию в новых версиях), пересохраните с
...
struct ImportedMaterial {
    ...
    // Как обращаться с альфой альбедо: 0 — никак, 1 — вырез по порогу
    // (листва, трава, решётки), 2 — смешивание. И видна ли поверхность с
    // изнанки. Это такое же описание материала, как цвет: без него карточка
    // листа приезжает непрозрачным квадратом и пропадает с обратной стороны.
    int AlphaMode = 0;
