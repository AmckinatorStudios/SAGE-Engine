...
// (3ds Max, старые экспортёры). Приведение к системе движка обязано быть ОДНИМ на
...
struct MeshCorner {
    ...
    // 3D-редактор, и без этого номера вся модель красилась первым материалом.
    int Material = 0;
};
...
// постповорот и опорные точки, и пропустить их нельзя. Maya, 3ds Max и сервисы риггинга
