...
public:
    ...
    // Тема ИНСТРУМЕНТА: редактор и его окна. Плотная, холодная, почти без
    // скруглений; золото SAGE — только для выбранного и нажимаемого.
    //
    // Отдельной темой, а не «настройками поверх игровой», по §19: игра и
    // редактор используют ОДИН фреймворк и РАЗНЫЕ темы, и это единственный
    // способ не тащить в редактор игровой воздух, а в игру — редакторскую
    // плотность.
    static UITheme Editor();
