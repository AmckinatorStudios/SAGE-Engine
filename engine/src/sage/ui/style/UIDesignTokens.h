...
public:
    ...
    // Набор для ИНСТРУМЕНТОВ: редактор и его окна. Отличается от игрового не
    // вкусом, а задачей — на экране одновременно полторы сотни подписей, полей
    // и строк, и они обязаны читаться, а не украшать. Отсюда всё: холодный
    // почти чёрный фон, мелкий кегль, скругления в три пикселя и отступы вдвое
    // меньше игровых. Игровая тема с её воздухом в редакторе означала бы вдвое
    // меньше видимых строк в списке.
    static UIDesignTokens Editor();
