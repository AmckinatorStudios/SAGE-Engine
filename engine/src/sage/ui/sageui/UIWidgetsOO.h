...
// Значок в коде — перечислением, а не строкой (§9 ТЗ значков): опечатка в имени
// обязана быть ошибкой сборки. Строка остаётся только там, где значок приходит
// из ФАЙЛА — из документа интерфейса или от плагина.
...
public:
    ...
    // Ширина по подписи плюс поля. Нужна вкладкам и пунктам меню, где кнопки
    // стоят в ряд и обязаны быть разной ширины.
    //
    // Отдельным методом, а не «поставьте WidthMode = Content»: у кнопки текст
    // живёт в ДОЧЕРНЕМ узле, и одного режима оси мало — контейнеру нужен ещё и
    // флаг «умею считать содержимое», а подписи внутри нельзя оставаться
    // растянутой. Три связанных настройки, забыть любую из которых значит
    // получить кнопку нулевой ширины (то есть невидимую).
    Button* FitToText(float padX = 10.0f);
