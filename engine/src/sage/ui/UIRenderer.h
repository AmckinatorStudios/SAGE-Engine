...
struct UITextStyle {
    ...
    bool Bold = false;
    bool Italic = false;
    // Округлять масштаб глифа до целого (пиксельные шрифты). Отдельно от
    // фильтрации: фильтрация — чем сглаживать, а это — можно ли рисовать
    // пиксель шрифта дробным числом экранных. Ценой идут ступени размера,
    // поэтому включается по просьбе, а не само.
...
public:
    ...
    //
    // Живёт в движке, а не в редакторе: Редактор вёрстки рисовал интерфейс
    // ровно в размер вьюпорта, пиксель в пиксель. Из этого следовало главное
    // неудобство: за краем экрана НИЧЕГО НЕ ВИДНО. Элемент, уехавший за границу
    // (а уехать ему проще простого — сменили якорь, перетащили мимо), исчезал
    // насовсем: его не видно, значит не выделить мышью, значит не вернуть. И
    // «отдалить, чтобы посмотреть, что там за краем» было нечем.
    //
    // Своими силами редактор этого сделать не может: интерфейс рисует движок, и
    // масштабировать уже готовый кадр нельзя — получилось бы мыло вместо
    // резких букв. Поэтому масштаб входит в саму матрицу вывода: текст и рамки
    // остаются резкими на любом приближении.
    //
    // originPx — где в кадре окажется левый верхний угол экрана интерфейса;
    // scale — во сколько раз он ужат (0.5 — половина, вокруг остаётся поле, в
    // котором видно всё, что вышло за границу). fbWidth/fbHeight — размер
    // кадра, в который рисуем; по умолчанию совпадает с экраном.
    void SetView(glm::vec2 originPx, float scale, int fbWidth, int fbHeight);
    void ResetView() { SetView(glm::vec2(0.0f), 1.0f, m_screenWidth, m_screenHeight); }
    ...
    // --- Поворот ------------------------------------------------------------
    //
    // Поворот применяется НА ВЫХОДЕ, к вершинам, а не к прямоугольникам на
    // входе. Причина простая: рисовальщиков у элемента много (подложка, рамка,
    // тень, картинка, глифы текста, полоса, значок), и каждый считает свою
    // геометрию сам. Заставить каждого знать про поворот значит переписать их
    // все и получить семь мест, где про него могут забыть. Здесь место одно —
    // и повёрнутым оказывается ВСЁ, что элемент нарисовал, включая текст.
    //
    // Вложенность не поддерживается намеренно: поворот внутри поворота — это
    ...
    float FontScale(float scale, const Font* font = nullptr, bool snapPixels = false) const;
    // Высота строки в пикселях при данном масштабе (для вертикальной вёрстки).
    //
    // НОМИНАЛ И РЕАЛЬНОСТЬ ТЕПЕРЬ СОВПАДАЮТ: кегль нормируется по высоте
    // строки шрифта (см. FontScale), поэтому строка на экране занимает ровно
    // столько. Пока они расходились, вёрстка считалась по одному числу, а
    // рисовалась по другому — надпись съезжала вверх тем сильнее, чем крупнее
    // кегль, и центрировать её было нечем.
...
private:
    ...
    // Поворачивает точку текущим поворотом (без поворота — отдаёт как есть).
