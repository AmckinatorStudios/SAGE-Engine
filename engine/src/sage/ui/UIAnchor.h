...
// ---------------------------------------------------------------------
// ПРЯМОУГОЛЬНИК ИНТЕРФЕЙСА И ДЕВЯТЬ КЛАССИЧЕСКИХ ЯКОРЕЙ.
//
// Лёгкий заголовок без зависимостей рендера — его включает всё, что считает
// вёрстку.
//
// ЯКОРЬ ЗДЕСЬ — НЕ МОДЕЛЬ, А СЛОВАРЬ. Новая система интерфейса привязывает узлы
// ДОЛЯМИ родителя (UITransform::AnchorMin/AnchorMax): доли выражают и эти девять
// точек, и всё, чего перечислением не выразить, — «правая треть экрана», «от 20%
// до 80% ширины». Перечисление осталось затем, что человеку удобно сказать
// «правый верхний угол», а не «(1, 0)»: UITransform::SetAnchorPoint переводит
// одно в другое.
// ---------------------------------------------------------------------
