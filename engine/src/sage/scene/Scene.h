...
public:
    ...
    // ШИНА СОБЫТИЙ СЦЕНЫ. Принадлежит именно сцене, а не движку целиком: при
    // загрузке следующего уровня подписки предыдущего обязаны исчезнуть вместе
    // с ним. Глобальная шина держала бы обработчики выгруженного уровня — и
    // событие «игрок вошёл в зону» звало бы код мёртвой сцены.
    //
