...
//
// ЛЕЖИТ В scene/, А НЕ В ui/, и это не перестановка файлов. Здесь ИСТОРИЯ
// ФОРМАТА СЦЕН — ровно то же, что миграции (SceneMigrations.h), и читает эту
// структуру ровно один потребитель: разбор старого .sage. Пока она лежала в
// ui/, всякий, кто открывал папку интерфейса, видел там перечисление видов
// элемента (панель, надпись, картинка, полоса, значок, поле, галка, ползунок) —
// то самое, чего в системе интерфейса нет и быть не должно. Соблазн «а можно я
// возьму отсюда Kind» возникает от одного соседства.
//
