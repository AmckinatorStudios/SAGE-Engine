...
public:
    ...
    // Сколько событий очередь готова держать между кадрами. Предел нужен не от
    // потока событий (его не бывает), а от ХОЗЯИНА, который перестал открывать
    // кадр: редактор, где панель Game закрыта, окно, свёрнутое на час. Движения
    // мыши идут сотнями в секунду, и очередь, которую никто не разбирает,
    // съедает память молча. Переполнение — это ошибка вызывающего, поэтому оно
    // пишется в лог, а не проглатывается.
    static constexpr size_t kMaxPendingEvents = 8192;
...
private:
    ...
    bool m_overflowReported = false;  // о переполнении очереди говорим один раз
