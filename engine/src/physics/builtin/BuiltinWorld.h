...
private:
    ...
    struct ContactConstraint {
        ...
        // Тела пары — УКАЗАТЕЛЯМИ, найденными один раз при постройке контакта.
        //
        // Здесь стоял поиск по ключу в хэш-карте тел, и он делался заново на
        // каждой итерации решателя: десять итераций на подшаг, две штуки на
        // контакт, сотни контактов — десятки тысяч хэш-поисков за подшаг ради
        // указателя, который не менялся с момента постройки. Это и было
        // основной ценой шага, а не сама математика.
        //
        // Указатели живут до конца подшага по построению: тела внутри SubStep
        // не создаются и не удаляются, а узловое хранение хэш-карты не двигает
        // существующие элементы даже при перестройке таблицы.
        Body* A = nullptr;
        Body* B = nullptr;
    };
