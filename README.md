...
[**The Boat**](https://github.com/AmckinatorStudios/The-Boat) — воксельная игра
на выживание от первого лица. Живёт отдельно от движка и лежит НЕ в `games/`, а
в своём репозитории — как обычный проект редактора (`project.sageproj` +
`scenes/` + `assets/`). Это принципиально: `sandbox` и `testgame` собираются
вместе с движком и линкуют его как библиотеку, а The Boat не содержит НИ СТРОЧКИ
C++ — она открывается редактором, играется в Play и собирается в exe кнопкой
File → Build Game, как любая игра стороннего разработчика.

Воксельный мир, обзор от первого лица, добыча и постройка блоков, шкалы
выживания, смена суток и весь интерфейс написаны на Lua поверх обычной ECS: в
движке нет ни одной строчки, знающей про воксели. Игра оказалась самой честной
проверкой границы «движок / игра» из всех — и первым потребителем, которому
обычного скриптинга не хватило.

Что она заставила добавить в движок (подробности — в разделах «Скриптинг» и
«CI и smoke-тесты»):
...
- `LaunchArg`/`LaunchFlag` — зерно мира и режим автопрогона снаружи;
