...
так что анимированные и статичные меши освещаются ОДИНАКОВО (без рассогласования).
Модель — разделяемый ассет (скелет/клипы), а покадровое состояние держит
`Animator` у каждой сущности.
