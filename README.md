...
- **Полное освещение в редакторе**: Viewport и Game рендерятся lit-шейдером
  движка — все стандартные типы света для forward-рендера:
...
- **Инстансный батчинг.** Видимые сущности группируются по мешу и рисуются
  ...
  матрица + цвет, loc 3..7), заливаемый раз в кадр, и `DrawInstances` →
  `glDrawElementsInstanced`. N одинаковых кубов = 1 draw call вместо N.

Освещение инстансного прохода идентично статическому `lit.frag` (ambient/солнце
с тенями/точечные/прожекторы/туман). Подключено в редакторе (Viewport и Game),
...
GPU. Его **фрагментная стадия идентична статическому `lit.frag`** — ambient
(в т.ч. из скайбокса) + солнце с PCF-тенями + точечные + прожекторы + туман, —
так что анимированные и статичные меши освещаются ОДИНАКОВО. Скелетные модели
