...
SAGE-Engine/
  ...
  engine/                     → библиотека sage::engine (универсальное ядро)
    ...
    src/sage/                 — ПУБЛИЧНЫЙ API (включается как <sage/...>)
      ...
      physics/  PhysicsWorld (абстракция) — Jolt / Builtin / Null бэкенды
...
**Крупную игру** (не showcase, а полноценный продакшен-проект) собирают тем же
`sage_add_game()`, только из многих систем и сцен. Как именно спроектировать и
пофазово построить такую игру на текущих возможностях движка — разобрано на
конкретном концепте в [`docs/PRODUCTION_GAME_PLAN.md`](docs/PRODUCTION_GAME_PLAN.md)
(экшн-адвенчура «Emberfall»: маппинг механик на подсистемы SAGE, структура
`games/emberfall/`, кастомные ECS-компоненты, роадмап играемыми срезами).
...
- **Jolt** (`Backend::Jolt`) — основной, «взрослый» бэкенд поверх
  ...
  когда собран.
- **Builtin** (`Backend::Builtin`) — встроенный лёгкий бэкенд без внешних
  зависимостей: честный импульсный решатель (sequential impulses) с контактом
  **динамика-динамика** и динамика-статика — тела реально расталкиваются и
  складываются в стопки, тяжёлое давит на лёгкое (позиционная коррекция по
  обратной массе), есть упругость и кулоновское трение; формы — настоящая
  сфера (сфера-сфера, сфера-бокс) и AABB-бокс (капсула/составное тело
  приближаются AABB). Сознательно без вращательной динамики и соединений
  (joints/ragdoll) — это область Jolt. Доступен всегда — на нём движок
  собирается и работает даже с `-DSAGE_PHYSICS_JOLT=OFF` (быстрая сборка,
  окружения без сети). Fallback, если Jolt не скомпилирован.
...
- **Физика времени выполнения** (после `BindPhysics`, привязан в Play-режиме
  ...
  **рейкасты** — `Raycast(origin, dir [, maxDist])` возвращает `{position,
  normal, distance, entity, entityId}` или `nil` (прицеливание, проверка
  земли под ногами, лучи взгляда ИИ); **события столкновений** — хуки
  `OnCollisionEnter(entity, other)` / `OnCollisionExit(entity, other)`
  (контакты собирают оба бэкенда физики: Jolt — ContactListener, Builtin —
  диф пар касания по шагам).
- **Горячая перезагрузка**: движок раз в секунду проверяет mtime привязанных
  `.lua` и перечитывает изменившиеся НА ЛЕТУ (OnStart вызывается заново,
  локальное состояние окружения скрипта сбрасывается) — правишь скрипт,
  поведение меняется без перезапуска игры/Play.
- **Сеть**: таблица `Net.*` и хуки `OnNetMessage`/`OnClientConnected`/... —
  см. раздел «Мультиплеер».
...
полностью прокачиваемый из главного цикла (без фоновых потоков) и доступный
из Lua:

- **Транспорт** — собственный протокол поверх UDP (`UdpSocket` — единственное
  место с системными заголовками сети, POSIX/Winsock): пакеты с seq/ack/
  ack-битами, НАДЁЖНЫЙ канал (гарантированная доставка В ПОРЯДКЕ отправки:
  msgId, переотправка по таймеру, дедупликация и сборка на приёме) и
  ненадёжный (свежесть важнее доставки — снапшоты). Сообщения больше MTU
  прозрачно фрагментируются и собираются (до 256 КБ). Keepalive/таймауты,
  handshake подключение/отказ (сервер заполнен), выдача clientId.
- **Репликация сцены** (`NetworkSystem`): сущности с `NetReplicatedComponent`
  сервер рассылает снапшотами 20 раз/с (позиция/поворот/масштаб/цвет/
  меш-примитив); клиент обновляет сущности С ТЕМ ЖЕ id (кооп: обе стороны
...
- **Lua API** (после `BindNetwork`; в SagePlayer и Play-режиме редактора —
  привязан из коробки): `Net.Host(port)`, `Net.Connect(ip, port)`,
  `Net.IsServer/IsClient/IsConnected`, `Net.Send(name, data)` (клиент->сервер),
  `Net.SendTo(clientId, ...)`/`Net.Broadcast(...)` (сервер), `Net.Clients()`.
  `data` — любое Lua-значение (число/строка/таблица, рекурсивно) — кодируется
  компактным бинарным кодеком. Хуки скриптов: `OnNetMessage(name, data,
  senderId)`, `OnClientConnected(id)`, `OnClientDisconnected(id)`,
  `OnNetConnected()`, `OnNetDisconnected()`.
...
Покрытие тестами (`tests/test_net.cpp`) — на НАСТОЯЩИХ UDP-сокетах
(loopback, headless): подключение/обмен/отказ при заполнении, доставка и
порядок при 35% симулированных потерь в обе стороны, фрагментация 10 КБ,
жизненный цикл реплицируемого «призрака» (спавн/движение/удаление) и полный
путь Lua->сеть->Lua. Часы тестов — ручные: никакого wall-clock фларика.
...
- **Физика — полноценная (Jolt), с выбором бэкенда.** Основной — **Jolt**,
  плюс встроенный `Builtin` (без зависимостей) и `Null` (выкл). Твёрдые тела
  ...
  (compound-части, Joint-компонент) и сериализуется в `.sage`. На `Builtin`-
  ...
  триггеры/сенсоры. Рейкасты и события контактов ПРОБРОШЕНЫ в абстракцию
  (`PhysicsWorld::Raycast`/`DrainContactEvents`, оба бэкенда) и в Lua
  (`Raycast`, `OnCollisionEnter/Exit`).
