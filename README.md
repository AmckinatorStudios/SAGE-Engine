...
- **Полное освещение в редакторе**: Viewport и Game рендерятся lit-шейдером
  движка — hemisphere ambient + солнце + точечные света + PCF-тени
  (общий shadow-проход на кадр). Панель **Lighting** правит окружение сцены
  (ambient/солнце — сериализуются со сценой, с undo); точечные света — это
  СУЩНОСТИ с движковым `LightComponent` (Color/Intensity/Range, позиция из
  Transform — двигаются гизмо и скриптами): Entity > Create Light, секция
  Light в Inspector; у выбранного света рисуется сфера радиуса затухания.
  Итоговое освещение кадра собирает `sage::ecs::CollectLighting` (окружение +
  света-сущности, лимит шейдера — 8 точечных).
...
- **Материалы**: `.sagemat` (JSON: albedo/emissive/shininess/texture) —
  переиспользуемое описание внешнего вида. Клик по `.sagemat` в Assets
  открывает его редактор в Inspector (правки видны на всех сущностях с этим
  материалом сразу — экземпляр разделяемый, Save пишет на диск); у выбранной
  сущности секция Material — Assign/Clear. Назначенный материал заменяет Color
