...
клипа. На этом стоит ручная анимация персонажа в Director 3D, и на это же
ляжет будущий IK: солверу останется посчитать углы. Наружу отдаются и
...
- Хот-релоад скриптов; каскадные тени (CSM) + мягкие тени (PCSS);
  ...
  обратная кинематика (IK) поверх готового слоя ручной позы; буфер скоростей
  (motion vectors) для честного смаза от объектов и для TAA.
