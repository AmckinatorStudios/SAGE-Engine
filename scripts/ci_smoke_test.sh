...
# запускаются, рисуют кадр и корректно завершаются (exit 0). Дополняет
# unit-тесты (tests/, цель sage_tests — 682 проверки без GL), но не заменяет их:
# здесь проверяется то, чего модульный тест не видит В ПРИНЦИПЕ — живой запуск,
# кадр, падение процесса и КОД ВОЗВРАТА после выхода.
#
...
# Манифест игры лежит В ПАКЕТЕ, а отдельной его копии рядом с exe нет: она
# ничего не даёт (плеер читает манифест из пакета), а человеку, собравшему игру
# в редакторе, этот файл и так знаком — он его автор. Включается параметром
# сборки (EngineConfig::BuildProjectFile), по умолчанию выключен.
if [ -f "${GAME_DIR}/project.sageproj" ]; then
...
echo "OK: манифест игры внутри пакета, отдельной копии рядом нет"

# И ИМЯ ИГРЫ ДОХОДИТ ДО ОКНА. Проверка не косметическая: пока манифест читался
# честным ifstream'ом, игра без отдельного файла называлась «SAGE Player» —
# то есть у всякой собранной игры пропадало бы имя.
if ! grep -qi "selftest_project" "${PLAYER_LOG}"; then
    echo "ОШИБКА: имя игры из манифеста в пакете не прочиталось"
    cat "${PLAYER_LOG}"; exit 1
fi
echo "OK: имя игры прочитано из манифеста в пакете"
...
echo "=== Smoke-тест 10/11: шаблон скачивается из каталога и ставится ==="
...
echo "=== Smoke-тест 11/11: выход из движка — код возврата ==="
# Падение при выходе случается ПОСЛЕ main: обработчик выхода разрушает
# статические объекты движка уже без графического контекста. В логе к тому
# моменту всё хорошо, отчёт напечатан, и наружу виден только код возврата — им и
# проверяем. Проба (tests/exit_probe.cpp) НАРОЧНО не зовёт
# ResourceManager::Clear() в OnDetach: она играет роль первой игры, написанной по
# документации, для которой обходной путь не обязателен.
EXIT_PROBE="$(cd "$(dirname "${BUILD_DIR}/tests/sage_exit_probe")" && pwd)/sage_exit_probe"
if [ ! -x "${EXIT_PROBE}" ]; then
    echo "ОШИБКА: ${EXIT_PROBE} не собран"; exit 1
fi
EXIT_DIR="${SCRATCH_DIR}/exit_probe"
rm -rf "${EXIT_DIR}"; mkdir -p "${EXIT_DIR}"
STATUS=0
( cd "${EXIT_DIR}" && run_headless "${EXIT_PROBE}" ) > "${EXIT_DIR}/probe.log" 2>&1 || STATUS=$?
if [ ${STATUS} -ne 0 ]; then
    echo "ОШИБКА: проба выхода завершилась кодом ${STATUS}; 139 — падение при"
    echo "        завершении процесса (139 = 128 + SIGSEGV). Хвост лога:"
    tail -20 "${EXIT_DIR}/probe.log"; exit 1
fi
echo "OK: движок завершился сам, без обходного Clear() (код 0)"
