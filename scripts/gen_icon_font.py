...
ICONS = {
    ...
    "unlock": "lock-open",
    # Вставка — БУФЕР ОБМЕНА. Пункт «Вставить» рисовался иконкой «copy», то есть
    # тем же рисунком, что и «Копировать» строкой выше: два разных действия
    # выглядели одинаково.
    "paste": "clipboard",
    # Выравнивание — ШЕСТЬ РИСУНКОВ Tabler вместо шести, нарисованных вручную
    # прямоугольниками. Рисовать рукой то, что в наборе уже есть, значит завести
    # седьмой вес штриха: в ряду с «сеткой» и «магнитом» самодельные плашки
    # видно сразу.
    "align-left": "layout-align-left", "align-center-x": "layout-align-center",
    "align-right": "layout-align-right", "align-top": "layout-align-top",
    "align-center-y": "layout-align-middle", "align-bottom": "layout-align-bottom",
    # Цвет подложки — образец краски: кнопка открывает выбор цвета, а не
    # материал (тот уже занял «palette»).
    "color": "color-swatch",
    # «Вписать кадр» — СООТНОШЕНИЕ СТОРОН: кнопка не увеличивает и не
    # уменьшает, она возвращает кадр целиком в панель, какой бы та ни стала.
    "fit": "aspect-ratio",
    # Якорь — ДЕВЯТЬ РИСУНКОВ «прямоугольник и в нём плашка у нужного края».
    # Раньше в кнопке сетки 3x3 рисовалась точка в углу: точка говорит «вот это
    # место», но не говорит, что элемент к этому месту ПРИЖМЁТСЯ, — а якорь
    # означает именно это. Центр — прицел: плашки по центру в наборе нет, а
    # «прямоугольник в прямоугольнике» читался бы как ещё один край.
    "anchor-tl": "box-align-top-left", "anchor-tc": "box-align-top",
    "anchor-tr": "box-align-top-right", "anchor-cl": "box-align-left",
